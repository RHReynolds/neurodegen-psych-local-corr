...
library(LAVA) # For LAVA-related functions
...
```{r chord-diagram, eval = F, fig.height = 6, fig.cap = "Chord diagram showing the number of significant (FDR < 0.05) bivariate local genetic correlations between each of the phenotypes across all loci."}

circos.par(
  gap.after = 
    setNames(
      object = rep(x = 2, 6),
      nm = results$univ$phen %>% levels()
    )
)
...
phen_count %>% 
  ...
  dplyr::arrange(phen1, phen2) %>% 
  dplyr::rename(from = phen1,
                to = phen2,
                value = n) %>% 
  circlize::chordDiagram(
    annotationTrack = "grid", 
    preAllocateTracks = 1, 
    grid.col = RColorBrewer::brewer.pal(n = 6, name = "BrBG")
    )
...
circlize::circos.trackPlotRegion(
  track.index = 1, panel.fun = function(x, y) {
    ...
    circlize::circos.axis(
      h = "top", 
      major.at = seq(from = 0, to = 50, by = 1),
      labels = FALSE,
      labels.cex = 0.5, 
      minor.ticks = 0,
...
```{r chord-diagram-rho, fig.height = 6, fig.cap = "Chord diagram showing the number of significant (FDR < 0.05) bivariate local genetic correlations between each of the phenotypes across all loci. Positive and negative correlations are coloured red and blue, respectively."}
data_to_plot <- 
  bivar_bonf %>% 
  dplyr::mutate(
    phen1 = phen1 %>% 
...
## Dissecting loci with signficant phenotype correlations {.tabset}

### Text
- The loci with the highest number of significant bivariate local $r_{g}$'s, locus 758 and 1719, were positioned on chromosome 4 and 11, respectively (Figure @\ref(fig:corr-plot)). 
...
```{r corr-plot, fig.height=16, fig.cap = "Heatmaps showing the standardised coefficient for genetic correlation (rho) for each association within the loci with significant bivariate local genetic correlations. Significant negative and positive correlations are indicated by blue and red fill, respectively. Non-significant correlations (FDR >= 0.05) have a grey fill. "}

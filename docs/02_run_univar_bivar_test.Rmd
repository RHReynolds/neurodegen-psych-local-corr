...
```{r comparison-to-prev-data, eval = F}
...
## Overlap with previous data {.tabset}

### Text

#### Gerring et al. 2022
- We intersected significant and non-significant bivariate local $r_{g}$s from this study that included neuropsychiatric disease traits (n, local $r_{g}$s = `r results$bivar %>% dplyr::filter(phen1 %in% c("BIP","MDD", "SCZ"), phen2 %in% c("BIP", "MDD", "SCZ")) %>% nrow()`; n, distinct LD blocks: `r results$bivar %>% dplyr::filter(phen1 %in% c("BIP","MDD", "SCZ"), phen2 %in% c("BIP", "MDD", "SCZ")) %>% dplyr::distinct(locus) %>% nrow()`; n, signficiant local $r_{g}$s = `r results$bivar %>% dplyr::filter(phen1 %in% c("BIP","MDD", "SCZ"), phen2 %in% c("BIP", "MDD", "SCZ"), p < bivar_thres) %>% nrow()`; n, distinct LD blocks with significant local $r_{g}$s = `r results$bivar %>% dplyr::filter(phen1 %in% c("BIP","MDD", "SCZ"), phen2 %in% c("BIP", "MDD", "SCZ"), p < bivar_thres) %>% dplyr::distinct(locus) %>% nrow()`) with previously reported local $r_{g}$s from a study of 10 psychiatric disorders and 10 substance abuse phenotypes (PMID: [35525699](https://pubmed.ncbi.nlm.nih.gov/35525699/)). 
- `r overlap_gerring %>% dplyr::filter(p.rhr < bivar_thres) %>% dplyr::distinct(locus.rhr) %>% nrow()` LD blocks were found to contain significant local $r_{g}$s from both studies, `r overlap_gerring %>% dplyr::filter(p.rhr < bivar_thres, overlapping_phen == 2) %>% dplyr::distinct(locus.rhr) %>% nrow()` of which contained the same phenotype pair (`r overlap_gerring %>% dplyr::filter(p.rhr < bivar_thres, overlapping_phen == 2) %>% dplyr::mutate(label = str_c("LD block ", locus.rhr, ", chr", chr, ":", start, "-", stop, ", ", phen1.rhr, " and ", phen2.rhr)) %>% dplyr::pull(label) %>% str_c(collapse = "; ")`) (\@ref(fig:plot-gerring-scatterplot)).
- Note: a total of 16 local $r_{g}$s (across 14 LD blocks) that included only BIP, MDD or SCZ were detected in Gerring et al. -- of these, we were able to perform bivariate tests using the same phenotype pairs in `r overlap_gerring %>% dplyr::filter(phen1.gerring %in% c("MDD", "BIP", "SCZ"), phen2.gerring %in% c("MDD", "BIP", "SCZ"), overlapping_phen == 2) %>% dplyr::distinct(locus.rhr) %>% nrow()` LD blocks, and `r overlap_gerring %>% dplyr::filter(p.rhr < bivar_thres, overlapping_phen == 2) %>% dplyr::distinct(locus.rhr) %>% nrow()` LD blocks contained `r overlap_gerring %>% dplyr::filter(p.rhr < bivar_thres, overlapping_phen == 2) %>% nrow()` local $r_{g}$s.
...
- `r overlap_smeland %>% nrow()` SNPs intersected with tested LD blocks, but `r overlap_smeland %>% dplyr::filter(p < bivar_thres) %>% nrow()` local $r_{g}$s within these blocks passed the threshold for significance (p < 0.05/`r nrow(results$bivar)`). Notably, in those LD blocks containing SNPs from Smeland et al. where allelic effect directions were consistent, a positive rho was observed in local $r_{g}$s, and likewise, in those LD blocks containing SNPs from Smeland et al. where allelic effect directions were inconsistent, a negative rho was observed in local $r_{g}$s.

### Table
```{r gerring-overlaps-table}

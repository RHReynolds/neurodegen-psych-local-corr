...
From these LD blocks of interest, we defined gene loci. That is, a 100 kb window was added to the start and end of any gene that overlapped a locus of interest. These gene-defined loci ($n$ = `r nrow(gene_loci)`) were carried forward in downstream analyses. Further, to ensure that our results were not driven by window size, we re-ran all analyses using a 50 kb window.
...
### Text
- Bivariate $r^{2}$, $\rho$ and -log10(p-value) estimates between phenotypes that could be tested using gene loci with a 50 kb or 100 kb window were significantly positively correlated across the two window sizes (Figure \@ref(fig:window-correlation)). Notably, $r^{2}$ estimates (i.e. the proportion of variance in genetic signal for one phenotype explained by the other) tended to be higher when using the 50 kb window, as compared to the 100 kb window, as evidenced by the fitted line falling below the equivalent of $y = x$.
- Using the more stringent Bonferroni cut-off, we detect a total of `r overlap %>% dplyr::filter(bonferroni_100000 < 0.05 & bonferroni_50000 < 0.05) %>% nrow()` significant bivariate local $r_{g}$'s across `r overlap %>% dplyr::filter(bonferroni_100000 < 0.05 & bonferroni_50000 < 0.05) %>% dplyr::distinct(gene_locus) %>% nrow()` distinct gene loci, which replicate irrespective of the window size used to define gene loci (Figure \@ref(fig:window-n)). 
...
## Phenotype correlations at gene locus level (100 kb window) using lenient FDR multiple test correction {.tabset}
...
- Locus 2351
    ...
    - A total of `r bivar_fdr$window_100000 %>% dplyr::filter(ld_block == 2351, str_detect(phen2, "ENSG")) %>% .[["gene_locus"]] %>% unique() %>% length()` gene loci contained a local $r_{g}$ between a disease phenotype and eQTL. 
...
## Phenotype correlations at gene locus level (100 kb window) using stringent bonferroni multiple test correction {.tabset}
